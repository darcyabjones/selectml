...
# TODO implement recursive feature selection with least squares.


class QROrthogonalSelector(SelectorMixin, BaseEstimator):

    """ Remove features that are identical using QR factorization.

    This removes anything that is exactly identical, but won't account well
    for imprecision. This should handle the marker matrices well though.
    """
    ...
    def fit(
        ...
        **kwargs
        ...
        # self.mask_ = np.abs(np.diag(r)) >= 1e-10

        rb = np.abs(r) < 1e-10
        # k=-1 keeps the diagonal
        rb[np.tril_indices_from(rb, k=-1)] = False
        self.mask_ = ~np.any(rb, axis=0)
        self.n_features_in_: int = X_.shape[1]
